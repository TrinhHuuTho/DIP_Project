...
def preprocess_frame(frame):
    ...
    # Convert back to BGR
    frame_eq = cv.cvtColor(equalized, cv.COLOR_GRAY2BGR)
    return frame_eq

def visualize(input, faces, fps, predictions=None, probabilities=None, thickness=2):
    if faces[1] is not None:
        for idx, face in enumerate(faces[1]):
            ...
            if predictions is not None and probabilities is not None and idx < len(predictions):
                label = f"{mydict[predictions[idx]]}: {probabilities[idx]*100:.2f}%"
                cv.putText(input, label, (coords[0], coords[1]-10), cv.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # Preprocess frame to improve lighting
        ...
        visualize(frame, faces, tm.getFPS(), predictions, probabilities)
